...
export default function AdminDashboard() {
  ...
  const statCards = [
    ...
    { label: 'Total Orders', value: data?.stats?.totalOrders?.toLocaleString(), icon: HiOutlineShoppingBag, color: 'from-purple-500/20 to-purple-600/10', iconColor: 'text-purple-500' },
    ...
    <main style={{ background: 'var(--bg-primary)', minHeight: '100vh' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <span className="font-mono text-xs text-purple-500 tracking-widest uppercase">Admin Panel</span>
        ...
        <div className="flex flex-wrap gap-3 mb-10">
          {navLinks.map(({ href, icon: Icon, label }) => (
            <Link key={href} href={href}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-purple-500/20 hover:border-purple-500/50 bg-purple-500/5 hover:bg-purple-500/10 text-sm font-semibold text-purple-500 transition-all">
              <Icon size={18} /> {label}
            </Link>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <Link href="/admin/orders" className="text-xs text-purple-500 hover:text-purple-400 font-semibold">View All →</Link>
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <Link href="/admin/users" className="text-xs text-purple-500 hover:text-purple-400 font-semibold">View All →</Link>
            </div>
            <div className="space-y-3">
              {data?.recentUsers?.length === 0 ? (
                ...
                <div key={u._id} className="flex items-center gap-3 py-2 border-b border-[var(--border)] last:border-0">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-400 to-purple-700 flex items-center justify-center text-obsidian-950 font-bold text-sm shrink-0">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                {data.categoryStats.map(({ _id, count }) => (
                  <div key={_id} className="p-3 rounded-xl bg-purple-500/5 border border-purple-500/10 text-center">
                    <p className="font-display text-2xl font-bold text-purple-500">{count}</p>
